...
void CoverageReductionPlugin::init(CSelectionData *data, String programName, String dirPath, unsigned int)
{
    m_data = data;
...
void CoverageReductionPlugin::coverageReduction(std::ofstream &outStream)
{
    ...
    std::set<IndexType> Tgen_rand, Tvec_rand;    // corresponding random test sets
    ...
    std::vector<IndexType> randomizedTests_gen;    // stores the shuffled test case id-s
    std::vector<IndexType> randomizedTests_vec;    // and its copy for the other method
    ...
    for(unsigned int i = 0; i < m_nrOfTestCases; i++) {
            ...
            randomizedTests_gen.push_back(i);
            addCoverage(totCoverage, m_data->getCoverage()->getBitMatrix().getRow(i));
    }
    unsigned int fullSize=To.size();
    shuffle(randomizedTests_gen); // make randomized order
    randomizedTests_vec = randomizedTests_gen; // make a copy
    ...
    CReductionData randomMatrix_gen(m_data->getCoverage(),m_programName + "-GEN-RANDOM", m_dirPath);
    ...
    CReductionData randomMatrix_vec(m_data->getCoverage(),m_programName + "-VEC-RANDOM", m_dirPath);
    ...
    // create random test suites of the same sizes:
    if (addRandom(randomizedTests_vec,Tvec_rand,Tvec.size()) != (int)Tvec.size()) // oops
      std::cerr << "[COVERAGE] random test addition failed (adding " << Tvec.size() << " tests from Vector-based strategy)" << std::endl;
    if (addRandom(randomizedTests_gen,Tgen_rand,Tgen.size()) != (int)Tgen.size()) // oops
      std::cerr << "[COVERAGE] random test addition failed (adding " << Tgen.size() << " tests from General strategy)" << std::endl;
    ...
    // save test matrices
    randomMatrix_gen.add(Tgen_rand);
    randomMatrix_gen.save(0);
    randomMatrix_vec.add(Tvec_rand);
    randomMatrix_vec.save(0);
    ...
    if (Tgen.size() != Tgen_rand.size()) // oops
      std::cerr << "[COVERAGE] random test size doeas not match with general test set size" << std::endl;
    outStream << std::endl;
    ...
    if (Tvec.size() != Tvec_rand.size()) // oops
      std::cerr << "[COVERAGE] random test size doeas not match with vector-based test set size" << std::endl;

    std::cerr << "done." << std::endl;
}

void CoverageReductionPlugin::shuffle(std::vector<IndexType>& v)
{
  srand(time(0));
...
int CoverageReductionPlugin::addRandom(std::vector<IndexType>& from, std::set<IndexType>& to, unsigned int N)
{
